	...
	<table id="gridTable2">
		<thead>
			<tr>
				...
				<th showColumn="name" sortColumn="NAME" width="250"><%=Role.ALIAS_NAME%></th>
				<th showColumn="organizationId" sortColumn="organizationId" width="250"><%=Role.ALIAS_ORGANIZATION_ID%></th>
				<th showColumn="enabled" sortColumn="ENABLED" width="100"><%=Role.ALIAS_WORK_GROUP_ID%></th>
			</tr>
		</thead>
		<tbody>
			<c:forEach items="${roleList}" var="item" varStatus="status">
				<tr>
					...
					<td showColumn="option">
						<a href="javascript:;"
							onclick="showPro('${ctx}/admin/auth/role/${item.id }/orgTabShow?parentType=org&orgId=${orgId }')"
					...
					<td showColumn="workGroupId">
						<c:out value='${item.workGroup.name}' />
						&nbsp;
					</td>
				</tr>
			</c:forEach>
