...
.role {
	...
	var setting = {
			...
			async: {
				enable: true,
				url:"${ctx}/admin/auth/organization/nodes",
		...
		function zTreeDblClick(e, treeId, treeNode){
			$.ajax({
			  type: "POST",
			  url: "${ctx}/admin/auth/organization/tab",
			  data: "id=" + treeNode.id,
			  dataType: "html",
			  cache: false,
...
<up72:override name="content">
	...
	<div class="head_content">
		<div class="navBar" style="display: none">
			»
			<a class="" href="${ctx}/admin/auth/product"><%=Organization.TABLE_ALIAS%>设置</a>
		</div>
	</div>
	<!-- END  当前位置 -->
	<!--end查询-->
	...
	<script type="text/javascript">
		$.ajax({
			url : "${ctx}/admin/auth/organization/dashboard",
			success : function(html){
				$("#editDiv").html(html);
			}
		});
		...
		function deleteOrganization(id){
			confirm("确认删除该机构吗？", function(){
	  		$.ajax({
	  			url : "${ctx}/admin/auth/organization/"+id+"/deleteOrganization",
	  			dataType : "json",
  	...
  	function deleteWorkGroup(id){
			confirm("确认删除该部门吗？", function(){
	  		$.ajax({
	  			url : "${ctx}/admin/auth/workGroup/"+id+"/deleteWorkGroup",
	  			dataType : "json",
	  			success : function(jsonDatas){
	  				alert("删除成功");
	  				window.location.reload();
		...
		confirm("确认删除该角色吗？", function(){
	  		$.ajax({
	  			...
	  			success : function(jsonDatas){
	  				...
	  				if(parentType=="wg")
	  				{
	  					getClass("work_group_tab3","${ctx}/admin/auth/workGroup/"+orgId+"/roleList");
	  				}else if (parentType=="org")
	  				{
	  					getClass("organization_tab3","${ctx}/admin/auth/organization/"+orgId+"/roleList");
	  				}
