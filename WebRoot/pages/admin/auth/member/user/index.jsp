  	...
  	<table id="gridTable">
		<thead>
			<tr>
				...
				<th showColumn="email" sortColumn="EMAIL" width="250" ><%=AuthUser.ALIAS_EMAIL%></th>
				<th showColumn="mobileValidate" sortColumn="MOBILE_VALIDATE" width="120"><%=AuthUser.ALIAS_MOBILE_VALIDATE%></th>
				<th showColumn="mobile" sortColumn="MOBILE" width="80"><%=AuthUser.ALIAS_MOBILE%></th>
				<th showColumn="enabled" sortColumn="ENABLED" width="50"><%=AuthUser.ALIAS_ENABLED%></th>
				<th showColumn="problem" sortColumn="PROBLEM" width="320"  style="display: none;"><%=AuthUser.ALIAS_PROBLEM%></th>
				<th showColumn="anser" sortColumn="ANSER" width="320"  style="display: none;"><%=AuthUser.ALIAS_ANSER%></th>
			</tr>
		</thead>
		<tbody>
	  <c:forEach items="${page.result}" var="item" varStatus="status">
			<tr rel="${ctx}/admin/auth/member/tab?id=${item.id}">
				...
				<td showColumn="option"><a href="javascript:;"  onclick="show('${ctx}/admin/auth/member/${item.id}/tabEdit','<%=AuthUser.TABLE_ALIAS%>',800)" class="sysiconBtnNoIcon">编辑</a>&nbsp;</td>
				...
				<td showColumn="code" style="display: none;"><c:choose><c:when test="${item.code == 1}">用户</c:when><c:when test="${item.code == 2}">管理员</c:when><c:when test="${item.code == 5}">系统管理员</c:when></c:choose>&nbsp;</td>
				<td showColumn="lastIp" style="display: none;"><c:out value='${item.lastIp}'/>&nbsp;</td>
				<td showColumn="lastVisiTime" style="display: none;"><fmt:formatDate value="${item.lastVisiTimeDate}" pattern="yyyy-MM-dd HH:mm:ss"/>&nbsp;</td>
				<td showColumn="email"><c:out value='${item.email}'/>&nbsp;</td>
				<td showColumn="mobileValidate" style="display: none;"><c:choose><c:when test="${item.mobileValidate == 0}">未认证</c:when><c:when test="${item.mobileValidate == 1}">已认证</c:when></c:choose>&nbsp;</td>
				<td showColumn="mobile" style="display: none;"><c:out value='${item.mobile}'/>&nbsp;</td>
				<td showColumn="enabled" style="display: none;"><c:choose><c:when test="${item.enabled == 0}">禁用</c:when><c:when test="${item.enabled == 1}">开启</c:when></c:choose>&nbsp;</td>
				<td showColumn="problem"><c:out value='${item.problem}'/>&nbsp;</td>
				<td showColumn="anser" ><c:out value="${item.anser}" /> &nbsp;</td>
			</tr>
		</c:forEach>
 	 	</tbody>
	</table>
...
<script type="text/javascript">
	...
	$('#gridTable').flexigrid({
		...
		striped : true,
		buttons : [
			{name: '添加', bclass: 'addorder', onpress : function(){show('${ctx}/admin/auth/member/user/new','<%=AuthUser.TABLE_ALIAS%>添加',600)}},
			{name: '删除', bclass: 'delete', onpress : function(){doRestEdit({confirmMsg:'确认删除选中的记录到回收站吗？',url:'${ctx}/admin/auth/member/recycleDelete?userType=user',batchBox:'items',boxCon:'#admin_auth_member_page_form',form:'#admin_auth_member_page_form',method:'delete'})}},
			{name: '回收站', bclass: '', onpress : function(){window.location.href="${ctx}/admin/auth/member/recycle?userType=user";}}
		]
	});
</script>
</up72:override>
