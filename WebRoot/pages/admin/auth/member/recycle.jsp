...
op_admin_member_recycleUndo = function(action,checkboxName,form){
...
<div class="up72_search">
  <form method="get" id="admin_auth_member_search_form" name="admin_auth_member_search_form" action="${ctx}/admin/auth/member/recycle">
  	...
  	<table id="gridTable">
		<thead>
			<tr>
				...
				<th showColumn="mobileValidate" sortColumn="MOBILE_VALIDATE" width="100"><%=AuthUser.ALIAS_MOBILE_VALIDATE%></th>
...
<script type="text/javascript">
	...
	$('#gridTable').flexigrid({
		...
		striped : true,
		buttons : [
			{name: '彻底删除', bclass: 'delete', onpress : function(){doRestBatchDelete('${ctx}/admin/auth/member?userType=${userType}','items',document.forms.admin_auth_member_page_form)}},
			{name: '恢复', bclass: '', onpress : function(){op_admin_member_recycleUndo('${ctx}/admin/auth/member/recycleReductive?userType=${userType}','items',document.forms.admin_auth_member_page_form)}},
			{name: '返回', bclass: '', onpress : function(){window.location.href="${ctx}/admin/auth/member/${userType}";}}
		]
	});
</script>
</up72:override>
