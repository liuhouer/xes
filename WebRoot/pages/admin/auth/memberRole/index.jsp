...
<title><%=MemberRole.TABLE_ALIAS%> 维护</title>
	...
	<script type="text/javascript" >
		$(document).ready(function() {
			// 分页需要依赖的初始化动作
			window.simpleTable = new SimpleTable('admin_auth_memberRole_page_form',${page.thisPageNumber},${page.pageSize},'${pageRequest.sortColumns}');
		});
	</script>
...
<form id="admin_auth_memberRole_page_form" name="admin_auth_memberRole_page_form" method="get">
  <div class="head_content">
    <!--当前位置-->
   <div class="navBar"> <a href="#">桌面</a> » <a class="" href="${ctx}/admin/auth/memberRole"><%=MemberRole.TABLE_ALIAS%>管理</a> </div>
    <!--end当前位置-->
    <!--查询-->
      ...
      <div class="headContent">
        <div class="finder_head">
          ...
          <div class="span_3" title=""> 操作</div>
            <!-- 排序时为th增加sortColumn即可,new SimpleTable('sortColumns')会为tableHeader自动增加排序功能; -->
				 <div class="span_4" title="" sortColumn="MEMBER_ID" ><%=MemberRole.ALIAS_MEMBER_ID%></div>
				 <div class="span_4" title="" sortColumn="ROLE_ID" ><%=MemberRole.ALIAS_ROLE_ID%></div>
        </div>
        ...
        <div id="list_button_rel" style="position:absolute; z-index:65535; right:0pt; padding:5px; display:none; border:1px solid #ccc;" class="finder_colChanger">
          ...
          <div style="overflow-y:auto;" class="x_colbody">
				 <div>
		              <label><input type="checkbox"  value="memberId" name="show_lable_value"><%=MemberRole.ALIAS_MEMBER_ID%></label>
           		 </div>
				 <div>
		              <label><input type="checkbox"  value="roleId" name="show_lable_value"><%=MemberRole.ALIAS_ROLE_ID%></label>
           		 </div>
          </div>
	...
	<!--- item list start --->
	  ...
	  <c:forEach items="${page.result}" var="item" varStatus="status">
		...
		<!-- item1 simple info start -->
          <div class="row_line hand" rel="${ctx}/pages/admin/auth/memberRole/tab.jsp?id=${item.id}"> 
            ...
            <div class="span_3"><a href="javascript:;"  onclick="show('${ctx}/admin/auth/memberRole/${item.id}/edit','<%=MemberRole.TABLE_ALIAS%>',600)" class="sysiconBtnNoIcon">编辑</a>&nbsp;</div>
