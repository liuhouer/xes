...
<form id="admin_auth_role_page_form" name="admin_auth_role_page_form">
	<input type="hidden" id="return_url" name="return_url"
		value="redirect:/pages/admin/auth/product/tab.jsp?id=${product.id}&AUTH_PERM_ID=${AUTH_PERM_ID}" />
	<table id="gridTable2">
		<thead>
			<tr>
				...
				<th showColumn="enabled" sortColumn="ENABLED" width="100"><%=Role.ALIAS_ENABLED%></th>
			</tr>
		</thead>
		<tbody>
			<c:forEach items="${product.roleList}" var="item" varStatus="status">
				<tr>
					...
					<td showColumn="option">
						<a href="javascript:;"
							onclick="showPro('${ctx}/admin/auth/role/${item.id }/proTabShow?AUTH_PERM_ID=${AUTH_PERM_ID}&productId=${product.id }')"
