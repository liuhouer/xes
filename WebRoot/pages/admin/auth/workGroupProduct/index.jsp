...
<title><%=OrganizationProduct.TABLE_ALIAS%> 维护</title>
	...
	<script type="text/javascript" >
		$(document).ready(function() {
			// 分页需要依赖的初始化动作
			window.simpleTable = new SimpleTable('admin_auth_organizationProduct_page_form',${page.thisPageNumber},${page.pageSize},'${pageRequest.sortColumns}');
		});
	</script>
...
<form id="admin_auth_organizationProduct_page_form" name="admin_auth_organizationProduct_page_form" method="get">
  <div class="head_content">
    <!--当前位置-->
   <div class="navBar"> <a href="#">桌面</a> » <a class="" href="${ctx}/admin/auth/organizationProduct"><%=OrganizationProduct.TABLE_ALIAS%>管理</a> </div>
    <!--end当前位置-->
    <!--查询-->
      ...
      <div class="headContent">
        <div class="finder_head">
          ...
          <div class="span_3" title=""> 操作</div>
            <!-- 排序时为th增加sortColumn即可,new SimpleTable('sortColumns')会为tableHeader自动增加排序功能; -->
				 <div class="span_4" title="" sortColumn="ORGANIZATION_ID" ><%=OrganizationProduct.ALIAS_ORGANIZATION_ID%></div>
				 <div class="span_4" title="" sortColumn="PRODUCT_ID" ><%=OrganizationProduct.ALIAS_PRODUCT_ID%></div>
        </div>
        ...
        <div id="list_button_rel" style="position:absolute; z-index:65535; right:0pt; padding:5px; display:none; border:1px solid #ccc;" class="finder_colChanger">
          ...
          <div style="overflow-y:auto;" class="x_colbody">
				 <div>
		              <label><input type="checkbox"  value="organizationId" name="show_lable_value"><%=OrganizationProduct.ALIAS_ORGANIZATION_ID%></label>
           		 </div>
				 <div>
		              <label><input type="checkbox"  value="productCode" name="show_lable_value"><%=OrganizationProduct.ALIAS_PRODUCT_ID%></label>
           		 </div>
          </div>
	...
	<!--- item list start --->
	  ...
	  <c:forEach items="${page.result}" var="item" varStatus="status">
		...
		<!-- item1 simple info start -->
          <div class="row_line hand" rel="${ctx}/pages/admin/auth/organizationProduct/tab.jsp?id=${item.id}"> 
            ...
            <div class="span_3"><a href="javascript:;"  onclick="show('${ctx}/admin/auth/organizationProduct/${item.id}/edit','<%=OrganizationProduct.TABLE_ALIAS%>',600)" class="sysiconBtnNoIcon">编辑</a>&nbsp;</div>
				 <div class="span_4"><c:out value='${item.organizationId}'/>&nbsp;</div>
				 <div class="span_4"><c:out value='${item.productCode}'/>&nbsp;</div>
          </div>
