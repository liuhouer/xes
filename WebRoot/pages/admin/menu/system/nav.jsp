...
<a href="${ctx}/admin/goProduct/${item.id}" class="dock-item2" title="${item.name }" ><span class="nav_ico${status.index}">${item.name }</span><img src="<c:choose><c:when test="${null!=item.imgPath && item.imgPath!=''}">${ctx}${item.imgPath}</c:when><c:otherwise>${ctx}/styles/default/skins/deepblue/images/icon_default.png</c:otherwise></c:choose>" /></a>
