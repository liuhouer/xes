...
<div class="workground">
  <div class="head_content">
    <div class="navBar"> <a href="#">桌面</a> » <a class="" href="${ctx}/admin/tools/file/img">文件管理</a> </div>
  ...
  <div>
   	<c:forEach items="${filePathList}" var="item" varStatus="status">
 		<a href="${ctx}/admin/tools/file/${type}?dir=${item.filePath}">${item.name}</a>
 	</c:forEach>
	<br />
	<a href="${ctx}/admin/tools/file/${type}?dir=${dir}">返回</a>
  </div>
  <!---文件夹遍历结束-->
      ...
      <div class="finder_list">
        <c:forEach items="${fileList}" var="item" varStatus="status">
          <div class="highlight_row row">
            <div class="row_line hand" rel="${ctx}/pages/admin/tools/file/tab.jsp?file=${item.filePath}">
              <div class="span_2">
                <input type="checkbox" name="items" value="${item.filePath}" class="sel" tags="null">
              </div>
              <div class="span_3">
			  <a href="javascript:;"  onclick="alert('编辑')" class="sysiconBtnNoIcon">编辑</a>&nbsp;
			  </div>
              <div class="span_4">
...
$(".kt_Mleft li").menuMouse({selCss:"on",overCss:"over",index:"/admin/tools/file/img"});
