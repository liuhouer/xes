...
$.fn.extend({
	permtree : function(options) {
		...
		this.init = function(){
			...
			//判断该checkbox是否选中
			if(!self.isNull(self.options.checked)){
				self.checkedBox();
			}
			//遍历树
			self.loopTree($(this));
		...
		this.checkedParentBox = function(box){
			...
			var parentBox = $(box).parent("span").parent("li").parent("ul").parent("li").children("span").children("input");
			$(parentBox).attr("checked","checked");
			var parentPaBox = $(parentBox).parent("span").parent("li").parent("ul").parent("li").children("span").children("input");
		...
		this.bindChangAction = function(li){
			$(li).children("span").children("input").bind("click",
				function(){
					var checked= $(li).children("span").children("input").attr("checked");
					if(isNull(checked) && !checked){
						checked = false;
					}else {
						checked = true;
					}
					self.selectParent(li,checked);
					self.toggleCheckChildren(li,checked);
				}
			);
		}
		
		//判断状态改变子类是否选中，选中也同时选中父类节点
		this.selectParent = function(li,checked){
			if($(li).children("span").children("input").attr("checked")){
				$(li).parents("li").children("span").children("input").attr("checked","checked");
			}
		}
		
		//激发chagn事件以后，反制触发事件的子节点的状态(checked->unchecked,unchecked->checked)
		this.toggleCheckChildren = function(li,checked){
			$(li).find("ul input").each(
				function(i,obj){
					if(checked){
		...
		this.selectInvert = function(li){
			...
			$(li).children("ul").each(
				function(i,obj){
					...
					$(obj).children("li").each(function(j,_obj){
						if(self.isNull($(_obj).children("ul"))
							|| $(_obj).children("ul").length == 0){
							if($(_obj).children("span").children("input").attr("checked")){
								$(_obj).children("span").children("input").attr("checked","");
							}else{
								isSelect = true;
								$(_obj).children("span").children("input").attr("checked","checked");
							}
						}
			...
			if(isSelect){
				$(li).children("span").children("input").attr("checked","checked");
			}else{
				$(li).children("span").children("input").attr("checked","");
			}
			return isSelect ;
		}
