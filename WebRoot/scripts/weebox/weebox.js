...
(function($) {
	...
	var weebox = function(content,options) {
		...
		this.initContent = function(content) {
			...
			if (self.options.type == 'suggest') {//例外处理
				...
				//ajax处理开始
					...
					self._content = "<iframe src='"+self.ajaxurl+"' width='100%' height='100%' frameborder='0' style='border:0px;display:block;'></iframe>";
					self.setContent(self._content);
				    self.show();
					self.focus();
				    self.onopen();
			...
			if (self.options.draggable && self.options.showTitle) {
				self.dh.find('.dialog-header').mousedown(function(event){
					...
					$(document).mousemove(function(event){
						...
						left = Math.min(left, bwidth-width);
						top = Math.min(top, bheight-height);
		...
		this.setCenterPosition = function() {
			...
			if(typeof(self.options.absHeight)!="undefined"
				&& self.options.absHeight!=null
				&& parseInt(self.options.absHeight)!="NaN"
				&& parseInt(self.options.absHeight)>0){
				pTop += parseInt(self.options.absHeight);
			}else{
