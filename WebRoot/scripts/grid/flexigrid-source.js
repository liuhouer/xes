...
(function($){
	...
	$.addFlex = function(t,p)
	{
		...
		var g = {
			...
			$(g.cDrag).css({top:g.hDiv.offsetTop/*+1*/});//do zhushi +1
			...
			$('thead tr:first th:visible',this.hDiv).each
				(
			 	function ()
					...
					$('div:eq('+n+')',g.cDrag).css({'left':(cdpos-1)+'px'}).show();//do tianjia -1
			...
			fixHeight: function (newH) {
					...
					$('div',this.cDrag).each(
						function ()
							{
								$(this).height(/*newH+*/hdHeight);//do zhushi newH
							}
					);
							...
							.click(
								function (e) 
									{ 
										...
										if(p.boxcheck && p.rowClick){
										$(this).toggleClass('trSelected');
										if (p.singleSelect) $(this).siblings().removeClass('trSelected');
										
										
										/**********        点击行，选中checkbox start        *********/
											var checkboxs = $(this).find("input[type='checkbox']");
											if(checkboxs.length > 0){
												if($(checkboxs[0]).attr("checked") == 'checked'){
													$(checkboxs[0]).removeAttr("checked");
												}else{
			...
			for (i=0;i<p.buttons.length;i++)
				{
					var btn = p.buttons[i];
					if(!btn.separator && 
						( typeof(btn.name)=="undefined" || btn.name==null )){
						continue ;
					}
					if (!btn.separator)
					{
						...
						.hover(
							function(){
								...
								if (g.colCopy) 
									...
									$(g.nBtn).css({'left':nl,top:g.hDiv.offsetTop,height:$(g.hDiv).height()}).show();
		...
		if ($('th',g.hDiv).length)
		{
			 	...
			 	function ()
					{
						...
						if(this.innerHTML.indexOf("input")!=-1){
							$('tbody',g.nDiv).append('<tr><td class="ndcol1"><input type="checkbox" '+ chk +' class="togCol" value="'+ cn +'" /></td><td class="ndcol2">全选操作</td></tr>');
						}else{
			...
			$('tbody',g.nDiv).append('<tr><td colspan="2"><div class="fbutton"><div><span onclick="$(this).savePageConfig(\'.ndcol1\');return false;" class="saveConfig">保存</span></div></div></td></tr>');
