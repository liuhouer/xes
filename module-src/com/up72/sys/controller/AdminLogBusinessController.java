...
@Controller
@RequestMapping("/admin/sys/logBusiness")
public class AdminLogBusinessController extends
		AuthController<LogBusiness, java.lang.Long> {
	...
	/** 列表 */
	@RequestMapping
	public String index(ModelMap model, LogBusinessQuery query,
		...
		UserDetails loginUser = this.getAdminLoginUser(request); // userUtils.getSessionUser(request,
																	// 0);
		if (null == loginUser) {
			String userName = userUtils.getLoginName(request,
					AuthConstants.MEMBER_TYPE_SYSTEM);
			if (null == userName || userName.trim().equals("")) {
				userName = userUtils.getLoginName(request,
		...
		if (null != loginUser) {
			...
			// params.put("organizationGuid",
			// loginUser.getOrganization().getOrganizationGuid());
			params.put("code", AuthConstants.MEMBER_TYPE_ADMIN);
			List<AuthUser> authUserList = authUserManager.findList(params, 0,
					null);
		...
		return "/admin/sys/logBusiness/index";
	}
	...
	public String _new(ModelMap model, LogBusiness logBusiness,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		model.addAttribute("logBusiness", logBusiness);
	...
	LogBusiness logBusiness, BindingResult errors, HttpServletRequest request,
	...
	public String deleteAll(ModelMap model, HttpServletRequest request,
			HttpServletResponse response) throws IOException {
		HashMap<String, Object> params = new HashMap<String, Object>();
		TreeMap<String, String> orderMap = new TreeMap<String, String>();
		orderMap.put("time", "desc");
		List<LogBusiness> logBusinessList = this.logBusinessManager.findList(
				params, 0, orderMap);
		UserDetails authUser = this.getAdminLoginUser(request); // userUtils.getSessionUser(request,
																// 0);//登录用户
		if (null == authUser) {
	...
	java.lang.Long[] items, HttpServletRequest request,
		...
		for (int i = 0; i < items.length; i++) {
			...
			systemLogUtil.delete(userGuid, request, "删除日志", LogBusiness
					.getDescription()
					+ " ID:" + items[i]);
		}
	...
	public String exportTxt(HttpServletRequest request,
			HttpServletResponse response) {
		String[] items = paramUtils.getParameters(request, "items", "");
		List<LogBusiness> logBusinessList = new ArrayList<LogBusiness>();
		if (null != items && items.length > 0) {
			...
			TreeMap<String, String> orderMap = new TreeMap<String, String>();
			orderMap.put("time", "desc");
			logBusinessList = this.logBusinessManager.findList(params, 0,
					orderMap);
		}
