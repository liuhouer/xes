...
@Controller
@RequestMapping("/jtzs/grade")
public class GradeController extends BaseRestSpringController<Grade,java.lang.Long>{
	//默认多列排序,example: username desc,createTime asc
	protected static final String DEFAULT_SORT_COLUMNS = null; 
	
	private GradeManager gradeManager;
	
	private final String LIST_ACTION = "redirect:/jtzs/grade";
	...
	public String index(ModelMap model,GradeQuery query,HttpServletRequest request,HttpServletResponse response) {
		Page page = this.gradeManager.findPage(query);
		
		model.addAllAttributes(toModelMap(page, query));
		return "/jtzs/grade/index";
	}
	...
	public String batchDelete(ModelMap model,@RequestParam("items") java.lang.Long[] items) {
		for(int i = 0; i < items.length; i++) {
			gradeManager.removeById(items[i]);
		}
		Flash.current().success(DELETE_SUCCESS);
		return LIST_ACTION;
	}
