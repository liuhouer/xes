...
@Controller
@RequestMapping("/jtzs/city")
public class CityController extends BaseRestSpringController<City,java.lang.Long>{
	//默认多列排序,example: username desc,createTime asc
	protected static final String DEFAULT_SORT_COLUMNS = null; 
	
	private CityManager cityManager;
	
	private final String LIST_ACTION = "redirect:/jtzs/city";
	...
	public String index(ModelMap model,CityQuery query,HttpServletRequest request,HttpServletResponse response) {
		Page page = this.cityManager.findPage(query);
		
		model.addAllAttributes(toModelMap(page, query));
		return "/jtzs/city/index";
	}
	...
	public String batchDelete(ModelMap model,@RequestParam("items") java.lang.Long[] items) {
		for(int i = 0; i < items.length; i++) {
			cityManager.removeById(items[i]);
		}
		Flash.current().success(DELETE_SUCCESS);
		return LIST_ACTION;
	}
