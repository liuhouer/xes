...
@Controller
@RequestMapping("/jtzs/province")
public class ProvinceController extends BaseRestSpringController<Province,java.lang.Long>{
	//默认多列排序,example: username desc,createTime asc
	protected static final String DEFAULT_SORT_COLUMNS = null; 
	
	private ProvinceManager provinceManager;
	
	private final String LIST_ACTION = "redirect:/jtzs/province";
	...
	public String index(ModelMap model,ProvinceQuery query,HttpServletRequest request,HttpServletResponse response) {
		Page page = this.provinceManager.findPage(query);
		
		model.addAllAttributes(toModelMap(page, query));
		return "/jtzs/province/index";
	}
	...
	public String batchDelete(ModelMap model,@RequestParam("items") java.lang.Long[] items) {
		for(int i = 0; i < items.length; i++) {
			provinceManager.removeById(items[i]);
		}
		Flash.current().success(DELETE_SUCCESS);
		return LIST_ACTION;
	}
