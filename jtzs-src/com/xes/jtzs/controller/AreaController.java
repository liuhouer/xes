...
@Controller
@RequestMapping("/jtzs/area")
public class AreaController extends BaseRestSpringController<Area,java.lang.Long>{
	//默认多列排序,example: username desc,createTime asc
	protected static final String DEFAULT_SORT_COLUMNS = null; 
	
	private AreaManager areaManager;
	
	private final String LIST_ACTION = "redirect:/jtzs/area";
	...
	public String index(ModelMap model,AreaQuery query,HttpServletRequest request,HttpServletResponse response) {
		Page page = this.areaManager.findPage(query);
		
		model.addAllAttributes(toModelMap(page, query));
		return "/jtzs/area/index";
	}
	...
	public String batchDelete(ModelMap model,@RequestParam("items") java.lang.Long[] items) {
		for(int i = 0; i < items.length; i++) {
			areaManager.removeById(items[i]);
		}
		Flash.current().success(DELETE_SUCCESS);
		return LIST_ACTION;
	}
